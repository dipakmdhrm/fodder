...
6. After the user merges, a release is cut **automatically**: `auto-release.yml`
   ...
   **skips tooling/docs-only merges** — a release fires only when the app crates
   ...
   `*.md`, or root scripts is skipped (a `release:*` bump label forces one). Label
...
- `fodder/`: the GTK4 viewer - 3-pane UI, the tokio<->glib bridge, the IPC client, and the light/WebKit reader. The WebKit view uses a dedicated `WebContext` and `terminate_web_process()` so its subprocesses' memory is reclaimed on toggle-back. The header-bar gear is a `MenuButton` (Preferences / About Fodder); About renders an `adw::AboutDialog`.
