...
- **`tray.rs`** - the `ksni` StatusNotifierItem tray (Open / Refresh all / Quit; left-click opens). Registration is **best-effort** - on hosts without an SNI tray (e.g. vanilla GNOME Shell) it logs and degrades, and the daemon keeps polling/notifying. Uses the themed `application-rss+xml` icon so it renders without our app icon installed. `run_supervised` then **keeps the icon alive across SNI-host restarts**: `ksni` re-registers on its own when the watcher's D-Bus name cycles on a live connection, but not when its own connection dies (a Wayland session crashing out from under a resident daemon), which used to strand the icon until a manual restart. The supervisor polls the `org.kde.StatusNotifierWatcher` owner on an **independent** `zbus` connection every 15s and re-`spawn`s the tray whenever a fresh host owner appears (relogin, shell restart, or a tray extension enabled after startup); the rebuild decision is the pure `tray_needs_rebuild`. A dedicated `tray_shutdown` `Notify` (separate from the single-waiter global `shutdown`) lets `main` drain the item off the bus during teardown/re-exec.
...
- **`fodderd/src/tray.rs`** - `tray_needs_rebuild`: rebuild when a host is present and either we hold no live item or the host's owner changed since last tick; never rebuild with no host, and don't flicker when the same host stays live. The `zbus` watcher probe and the supervisor loop are D-Bus glue, not unit-tested.
