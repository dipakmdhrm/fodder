...
- **`tray.rs`** - the `ksni` StatusNotifierItem tray (Open / Refresh all / Quit; left-click opens), driven by `run()`. Registration is **best-effort** - on hosts without an SNI tray (e.g. vanilla GNOME Shell) it logs and degrades, and the daemon keeps polling/notifying. The icon is our app PNG **embedded and decoded into ARGB `IconPixmap`s** with an empty `IconName` (several hosts, notably the GNOME AppIndicator extension, mishandle a themed `IconName` and show a placeholder instead of falling back to the pixmap). Recovery is **event-driven, not polled**: `ksni` re-registers the item whenever the `org.kde.StatusNotifierWatcher` restarts on the same bus, so a shell/watcher restart doesn't lose the icon. The one wrinkle is `self_update`'s re-exec (same PID), which releases and immediately re-requests the *same* `org.kde.StatusNotifierItem-<pid>-1` name - a handoff that can race the watcher's prune and drop us with no further signal. So `run()` does a single **deferred re-registration check** ~5s after startup: if our still-owned item isn't in the watcher's `RegisteredStatusNotifierItems` (resolving each entry's owner PID, host-agnostic), it re-adds it once via `RegisterStatusNotifierItem` (no tear-down, so no fresh race). A bounded one-shot, not a periodic poll. Uses `zbus` for the query and `png` to decode the embedded icon.
