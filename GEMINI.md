...
- `fodderd/`: the headless daemon - single-instance socket guard, IPC server, poll scheduler, batched actionable notifications + a daily reading reminder, the `ksni` tray (graceful-degrade where no SNI host exists) with **event-driven recovery**: ksni re-registers across watcher restarts (`assume_sni_available` tolerates autostarting before the host is up), the tray is re-established on a fresh connection when the D-Bus session connection dies (a logout/login severs it — detected via `zbus::Connection::closed()`), and a one-shot post-startup re-registration check neutralizes the `self_update` re-exec handoff (the app icon is embedded as ARGB pixmaps so hosts that mishandle a themed `IconName` still render it); plus on-demand viewer spawn/reap. Config is live-reloaded on a `ReloadConfig` message. The daemon also remembers the viewer's last-read article + mode (in memory) and restores it on the next open.
