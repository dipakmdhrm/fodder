...
- **`core/src/db/articles.rs`** - `INSERT OR IGNORE` dedupe (second insert returns no ids), cascade delete, unread counts + mark-read/mark-all, mark-read<->unread toggle, and listing. Plus the tombstones: a deleted article does not re-insert on the next poll, clearing a feed keeps the subscription and stays cleared, a tombstone is scoped to its own feed, and deleting a feed cascades its tombstones away.
...
- **`ArticleDaoTest`** (Robolectric, in-memory Room) - the `(feedId, guid)` dedupe returning no new ids on re-poll, the same guid in two feeds staying two articles, cascade delete, unread counts across mark-read/unread and mark-all, success clearing the error while storing validators, and `reschedule` preserving validators. It also covers the tombstones, mirroring the Rust tests one for one: a deleted article does not re-insert, a cleared feed stays cleared, a tombstone is scoped to its own feed, and deleting a feed cascades its tombstones away. It runs on a plain `Application`, not `FodderApp`, so booting it does not schedule WorkManager.
- **`MigrationTest`** (Robolectric) - the version 1 -> 2 upgrade: it hand-builds the v1 schema, opens the database at v2, and lets Room's own schema validation catch any drift between `MIGRATION_1_2`'s DDL and `DeletedArticleEntity`. This is the Android counterpart of the desktop's migration test; the other DAO tests use an in-memory database, which runs `createAllTables` and so never exercises a migration.
