...
- **`tray.rs`** - the `ksni` StatusNotifierItem tray (Open / Refresh all / Quit; left-click opens), driven by `run()`. Registration is **best-effort** - on hosts without an SNI tray (e.g. vanilla GNOME Shell) it logs and degrades, and the daemon keeps polling/notifying. The icon is our app PNG **embedded and decoded into ARGB `IconPixmap`s** with an empty `IconName` (several hosts, notably the GNOME AppIndicator extension, mishandle a themed `IconName` and show a placeholder instead of falling back to the pixmap). Recovery is **event-driven, not polled**, in three layers matched to how a daemon that outlives graphical sessions actually loses its icon: (1) **watcher restart** - `ksni` re-registers when `org.kde.StatusNotifierWatcher` reappears; we also pass `assume_sni_available(true)` so autostarting *before* the host is up isn't a hard error (ksni waits and registers when the watcher appears). (2) **session-bus connection death** - a full logout/login severs the D-Bus connection with an I/O error; the daemon survives but ksni's connection is dead and it doesn't reconnect (this is what makes the icon vanish after logout/login). `run()` holds its own connection on the same bus and awaits `zbus::Connection::closed()`; when it fires it re-establishes the tray on a fresh connection (retrying with backoff while the replacement bus comes up). (3) **re-exec handoff prune** - `self_update`'s re-exec (same PID) releases and re-requests the *same* `org.kde.StatusNotifierItem-<pid>-1` name, which can race the watcher's prune; so ~5s after each (re)spawn `run()` does a single **deferred re-registration check** - if our still-owned item isn't in the watcher's `RegisteredStatusNotifierItems` (resolving each entry's owner PID, host-agnostic), it re-adds it once via `RegisterStatusNotifierItem` (no tear-down, no fresh race). A bounded one-shot, not a periodic poll. Uses `zbus` for the query and `png` to decode the embedded icon.
