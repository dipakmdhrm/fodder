...
- `fodderd/`: the headless daemon - single-instance socket guard, IPC server, poll scheduler, batched actionable notifications + a daily reading reminder, the `ksni` tray (graceful-degrade where no SNI host exists, and self-healing across SNI-host restarts via a supervisor that re-registers the icon when a fresh watcher owner appears), and on-demand viewer spawn/reap. Config is live-reloaded on a `ReloadConfig` message. The daemon also remembers the viewer's last-read article + mode (in memory) and restores it on the next open.
